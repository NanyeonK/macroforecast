...
| `state_space_mixed_frequency_payload_v1` | Layer 3 | `owned` | future state-space nowcasting executor | artifact writer and evaluation | `gated_named` | Reserved contract for mixed-frequency state-space likelihoods. Before runtime opens, the package needs an upstream observation-equation schema, missing-observation handling policy, state/filter/smoother output payload, forecast-origin update semantics, and regression tests against a known implementation. Current `experiment_unit=state_space_run` remains a future design marker, not executable permission. |
