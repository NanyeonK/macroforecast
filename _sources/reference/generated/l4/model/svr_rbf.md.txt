...
Configures the ``model`` axis on ``L4_A_model_selection`` (layer ``l4``); the ``svr_rbf`` value is materialised in the recipe's ``fixed_axes`` block under that sub-layer.
...
Set ``params.model = "svr_rbf"`` in the relevant layer to activate this op within a recipe:
