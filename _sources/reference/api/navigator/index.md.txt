...
| L3 | Feature engineering steps, final features, feature metadata. |
| L4 | Forecast/model steps, forecasts, model artifacts, training metadata. |
| L5 | Evaluation metrics, aggregation, slicing, ranking. |
| L6 | Statistical tests over L4/L5 artifacts. |
| L7 | Interpretation and importance steps. |
...
```text
[L0 meta] → [L1 data] → [L2 clean] → [L3 features pipeline] → [L4 forecasts pipeline]
                                            ↓                      ↓
                                         L3.5 diag             L4.5 diag
[L4] → [L5 evaluation] → [L6 tests] → [L7 interpretation pipeline] → [L8 artifacts]
```
...
2. For L3/L4/L7, think in pipeline nodes: sources, steps, combines, and sinks.
