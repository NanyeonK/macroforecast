...
Quarterly U.S. macroeconomic panel maintained by the Federal Reserve Bank of St. Louis. Sister dataset to [FRED-MD](datasets_fred_md.md), designed for longer-horizon forecasting and for factor models requiring quarterly frequency (GDP, productivity, balance-sheet aggregates). Loaded via `macroforecast.load_fred_qd()` when `path.1_data_task.fixed_axes.dataset == "fred_qd"`.
...
- [FRED-MD](datasets_fred_md.md) — sister monthly database.
