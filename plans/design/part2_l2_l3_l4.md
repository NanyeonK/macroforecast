...
| Slot | 이름 | 답하는 질문 |
|---|---|---|
| L2.A | Mixed frequency alignment | mixed-frequency panel (FRED-SD or 사용자 panel)을 어떻게 맞출 것인가 |
...
**mixed_frequency_representation (L2.A)** — added v0.8.5

L2.A는 더 이상 FRED-SD 전용이 아니다. mixed-frequency 가 선언된 모든 panel(FRED-SD 또는 per-column native frequency가 metadata에 명시된 사용자 panel)에 적용된다. 기본은 ``calendar_aligned_frame`` 으로 종래의 alignment-rule 경로를 유지한다.

| 선택지 | 의미 |
|---|---|
| calendar_aligned_frame (default) | 선택된 mixed-freq 컬럼을 experiment calendar 위로 align |
| drop_unknown_native_frequency | native frequency 미상 컬럼 drop |
| drop_non_target_native_frequency | experiment frequency와 다른 컬럼 drop |
| native_frequency_block_payload | per-frequency block metadata 출력, panel은 untouched |
| mixed_frequency_model_adapter | block payload + L4 model-adapter contract (MIDAS-style) |

`native_frequency_block_payload` 와 `mixed_frequency_model_adapter` 는 L4 ``model_family`` 가 ``midas_almon`` / ``midasr`` / ``midasr_nealmon`` 또는 등록된 custom model 이며 ``auxiliary_payloads`` 를 소비할 때 사용한다.

**sd_tcode_policy (L2.B)** — added v0.8.5

`transform_policy` 와 직교하는 axis. FRED-SD 전용 t-code 정책을 별도로 켤 수 있다. 기본 ``none`` 은 FRED-SD source value 를 publish 그대로 둔다.

| 선택지 | 의미 |
|---|---|
| none (default) | FRED-SD-specific t-code 미적용 |
| inferred | 국가 analog 기반 reviewed t-code map (`official: false` manifest 기록) |
| empirical | stationarity audit 기반 t-code map (leaf_config.sd_tcode_unit 필요) |

`empirical` 모드는 leaf_config 에 ``sd_tcode_unit`` (`variable_global` 또는 `state_series`) 을 요구하며, ``state_series`` 는 추가로 ``sd_tcode_code_map`` 와 ``sd_tcode_audit_uri`` 를 요구한다.
...
| 조건 | 결과 |
|---|---|
| dataset에 fred_sd 미포함 | L2.A (mixed_frequency_representation 포함) + sd_tcode_policy 비활성 |
...
### Custom preprocessor hooks (pre vs post pipeline) — added v0.8.6

L2 는 두 개의 user-registered preprocessor hook을 지원한다. 둘 다
`macroforecast.custom.register_preprocessor` 로 등록한 callable을
`leaf_config` 의 단일 키 이름으로 가리키고, 둘 다 동일한
`(X_train, y_train, X_test, context) -> (X_train, X_test)` 계약을 따른다.
유일한 차이는 *언제* 호출되느냐다.

| Slot | 위치 | 입력 | 출력 |
|---|---|---|---|
| `leaf_config.custom_preprocessor` (pre-pipeline, v0.8.6) | transform / outlier / impute / frame_edge **이전** | L1 raw panel 그대로 | 정식 t-code 가 적용되기 전 단계의 panel |
| `leaf_config.custom_postprocessor` (post-pipeline, v0.2.5 #251) | 모든 stage **이후** | 정식 파이프라인이 끝낸 clean panel | L3 가 읽을 최종 clean panel |

Pre-pipeline hook 은 deflation, 통화 단위 변환, custom resampling 처럼
"공식 McCracken-Ng 파이프라인이 보기 *전에* 패널을 정리하고 싶다"는
경우에 쓴다. Post-pipeline hook 은 "정식 파이프라인 결과에 한 번 더
손을 대고 싶다" (예: 추가 winsorize, custom factor 추출)는 경우에 쓴다.
같은 이름을 양쪽에 동시에 routing 하는 것도 허용되며 cleaning_log 에
두 entry 가 분리되어 기록된다. Simple-API 의
`Experiment.use_preprocessor(name, applied_at='l2'|'l3')` 가 이 두
slot 으로 lower 된다.
