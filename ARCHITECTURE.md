...
> Last updated: C60 — Docs depth and polish: new `04_custom_preprocessor` tutorial (register_preprocessor API, context fields, recipe integration, debugging); `12_layer_design.md` terminology unified (13-slot count clarified); `bit_exact_replicate.md` caveats forward-reference added; MIDAS encyclopedia pages annotated with NLS vs OLS optimization method labels.
...
| `macroforecast/core/runtime.py` | Core | Per-layer `materialize_l*` artifact helpers; `_boruta_selection` | `materialize_l2` .. `materialize_l8` | yes (C59: Boruta fallback removed, n_shadow_copies=6) |
