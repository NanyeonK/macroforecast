...
> Last updated: 2026-05-26 — Phase 2 L1 axis deduplication (scriber). Removed 4 L1 axes (`official_transform_policy`, `official_transform_scope`, `raw_missing_policy`, `raw_outlier_policy`) and preprocessing parameter `tcode_application_scope`; deleted 2 architecture pages + 4 encyclopedia pages; updated 5 doc files. L1 now has 22 axes; single decision point for transform/imputation/outlier lives at L2. Phase 1 (2026-05-25): Phase 1 final docs cleanup (scriber-3). TAXONOMY v1 vocabulary applied: 13 axis renames, `core/dag.py` -> `core/pipeline.py`, L2 `*_scope` axes removed, `DEFAULT_MODEL_FAMILY` -> `DEFAULT_MODEL`. Phase 0 (2026-05-25): wizard subsystem removed. C67 (2026-05-23): R3-P4a tutorial standalone-first rewrite. C66: DAG jargon cleanup across public surface.
...
| `macroforecast/scaffold/` (8 files) | Scaffold | Encyclopedia generator + option doc sources | renders 321 pages | **yes (C66: 14 edits; C69-P0: wizard.py deleted)** |
