...
> Last updated: 2026-05-25 — Phase 1 final docs cleanup (scriber-3). Deleted 4 stale encyclopedia axis pages (`outlier_scope`, `transform_scope`, `imputation_scope`, `frame_edge_scope`); removed all cross-reference rows in `browse_by_axis`, `browse_by_option`, `l2/index.md`, architecture `layer2/index.md`. TAXONOMY v1 vocabulary applied: 13 axis renames, `core/dag.py` -> `core/pipeline.py`, L2 `*_scope` axes removed, `DEFAULT_MODEL_FAMILY` -> `DEFAULT_MODEL`. Phase 0 (2026-05-25): wizard subsystem removed. C67 (2026-05-23): R3-P4a tutorial standalone-first rewrite. C66: DAG jargon cleanup across public surface.
