...
> Last updated: C63 (2026-05-23) — R3-P1a standalone-first paradigm shift. Round 3 of cross-review remediation: 22 L4 model classes promoted to `mf.models.*`, 5 feature-selection class wrappers in `mf.feature_selection`, 1 transform function in `mf.transforms`, 2 interpretation wrappers in `mf.interpretation`, 8 new `*_fit` callables in `mf.functions`. Total: 38 new public exports across 4 new packages.
...
graph TD
    subgraph API["API Layer"]
        ...
        A3["macroforecast.functions — 126 callables"]
        A4["macroforecast.custom — register_model"]
        A5["macroforecast.models — 22 L4 classes"]
...
| Module / File | Layer | Purpose | Key Exports | Changed in C63 |
| --- | --- | --- | --- | --- |
...
| `macroforecast/functions/` | API | 126 standalone callables by layer (L2–L7) | layer-grouped callables | yes (C63: +8 MIDAS/ridge callables) |
| `macroforecast/functions/midas.py` | API | 4 MIDAS standalone fit callables | `midas_almon_fit`, `midas_beta_fit`, `midas_step_fit`, `unrestricted_midas_fit` | yes (C63: new) |
| `macroforecast/functions/ridge_variants.py` | API | 4 ridge-variant standalone fit callables | `nonneg_ridge_fit`, `random_walk_ridge_fit`, `shrink_to_target_ridge_fit`, `fused_difference_ridge_fit` | yes (C63: new) |
| `macroforecast/models/` | API | 22 promoted L4 model classes (thin subclasses) | `MidasAlmon`, `RealizedGARCH`, `BVAR`, etc. | yes (C63: new package) |
| `macroforecast/models/linear.py` | API | 14 linear/MIDAS/ridge-variant model classes | `MidasAlmon`, `MidasBeta`, `MidasStep`, `UnrestrictedMidas`, `LinearAR`, `FactorAugmentedAR`, `NonNegRidge`, `TwoStageRandomWalkRidge`, `ShrinkToTargetRidge`, `FusedDifferenceRidge`, `PrincipalComponentRegression`, `FactorAugmentedVAR`, `VAR`, `GLMBoost` | yes (C63: new) |
| `macroforecast/models/bayesian.py` | API | 3 Bayesian/DFM model classes | `BVAR`, `BVARMinnesota`, `DFMMixedFrequency` | yes (C63: new) |
| `macroforecast/models/volatility.py` | API | 2 volatility model classes | `GARCH`, `RealizedGARCH` | yes (C63: new) |
| `macroforecast/models/timeseries.py` | API | 3 time-series model classes | `ETS`, `Theta`, `HoltWinters` | yes (C63: new) |
| `macroforecast/feature_selection/` | API | 5 sklearn-style feature selection class wrappers | `Boruta`, `RFE`, `LassoPathSelector`, `StabilitySelection`, `GeneticSelection` | yes (C63: new package) |
| `macroforecast/transforms/` | API | Transform-only function wrappers | `chow_lin_disaggregate` | yes (C63: new package) |
| `macroforecast/interpretation/` | API | L7 interpretation class wrappers | `GIRF`, `LSTMHiddenState` | yes (C63: new package) |
| `macroforecast/__init__.py` | API | Package init, version declaration, lazy imports | `__version__` = `"0.9.3b1"`; `_LAZY_MODULES` extended with 4 new packages | yes (C63: lazy import wiring) |
| `macroforecast/py.typed` | API | PEP 561 typed-package marker (empty sentinel) | — | yes (C61: new file) |
...
| `macroforecast/core/runtime.py` | Core | Per-layer `materialize_l*` artifact helpers; `_boruta_selection` | `materialize_l2` .. `materialize_l8` | no (changed in C59) |
...
macroforecast/
  ...
  models/                 # C63: 22 promoted L4 model classes (thin subclasses)
    __init__.py           # flat re-export of all 22 classes
