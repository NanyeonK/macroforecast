...
> Last updated: C67 (2026-05-23) — R3-P4a. Tutorial 01-03 standalone-first rewrite: introductory tutorials now teach `mf.models` API first; recipes appear only in the graduation section. Net documentation reduction: 598 lines. Parameter names corrected: `LinearAR(p=2)`, `FactorAugmentedAR(p=2, n_factors=3)`, `predict(X)`. C66: DAG jargon cleanup across public surface (464 → 10 in macroforecast/, 131 → 7 non-archive in docs/).
...
| Module / File | Layer | Purpose | Key Exports | Changed in C66 |
| --- | --- | --- | --- | --- |
| `macroforecast/recipes/__init__.py` | API | **Canonical recipe orchestration namespace** — re-exports 8 symbols from api/api_high; `paper_methods` preserved | `run`, `run_file`, `replicate`, `ManifestExecutionResult`, `ReplicationResult`, `forecast`, `Experiment`, `ForecastResult`, `paper_methods` | no |
...
| `macroforecast/functions/` | API | 132 standalone callables by layer (L2–L7) | layer-grouped callables | **yes (C66: 28 docstring replacements — `recipe DAG` → `recipe pipeline` across 7 files)** |
...
| `macroforecast/functions/tree.py` | API | 5 tree-family standalone fit callables | `slow_growing_tree_fit`, `quantile_regression_forest_fit`, `bagging_fit`, `booging_fit`, `macro_random_forest_fit` | yes (C66: docstring cleanup) |
| `macroforecast/functions/deep.py` | API | 1 neural standalone fit callable | `hemisphere_nn_fit` | yes (C66: docstring cleanup) |
| `macroforecast/models/` | API | 30 promoted L4 model classes | `MidasAlmon`, `RealizedGARCH`, `BVAR`, `SlowGrowingTree`, `HemisphereNN`, etc. | no |
...
| `macroforecast/models/tree.py` | API | 6 tree-family model classes (BaseEstimator) | `SlowGrowingTree`, `QuantileRegressionForest`, `Bagging`, `Booging`, `MacroRandomForest`, `KNN` | no |
| `macroforecast/models/neural.py` | API | 2 neural model classes (BaseEstimator) | `SequenceModel`, `HemisphereNN` | no |
| `macroforecast/feature_selection/` | API | 5 sklearn-style feature selection class wrappers (BaseEstimator + TransformerMixin) | `Boruta`, `RFE`, `LassoPathSelector`, `StabilitySelection`, `GeneticSelection` | no |
...
| `macroforecast/__init__.py` | API | Package init, version declaration, lazy imports | `__version__` = `"0.9.3b1"`; `_LAZY_EXPORTS` routes 8 symbols via `.recipes` | **yes (C66: module docstring `DAG runtime` → `recipe runtime`)** |
...
| `macroforecast/core/runtime.py` | Core | Per-layer `materialize_l*` artifact helpers; `_boruta_selection`; `_SlowGrowingTree`; `_BaggingWrapper` | `materialize_l2` .. `materialize_l8` | no |
| `macroforecast/core/dag.py` | Core | Internal recipe graph (DAG) implementation | `DAG`, `DAGNode`, node types | **yes (C66: module docstring added — marks as internal-vocab)** |
| `macroforecast/core/layers/l3.py` | Core | L3 schema: feature pipeline step graph | layer axis / gate / validator | **yes (C66: 2 error messages → `step graph` terminology)** |
| `macroforecast/core/layers/l4.py` | Core | L4 schema: model step graph | layer axis / gate / validator | **yes (C66: 1 error message → `step graph` terminology)** |
| `macroforecast/core/validator.py` | Core | Recipe graph validation, cycle detection | `validate_dag`, `assert_valid_dag`, `DAGValidationError` | **yes (C66: 2 messages `DAG contains a cycle` → `Recipe graph contains a cycle`)** |
...
| `macroforecast/scaffold/` (6 files) | Scaffold | Encyclopedia generator + option doc sources | renders 321 pages | **yes (C66: 14 edits in generator sources; encyclopedia pages regenerated)** |
| `macroforecast/wizard/pages/layer_step_graph.py` | Wizard | Step graph placeholder page (was `layer_dag.py`) | `DAGPlaceholderPage` | **yes (C66: renamed + 5 UI strings → `step graph`)** |
| `macroforecast/wizard/pages/layer_form.py` | Wizard | Layer form renderer | form rendering | **yes (C66: import + docstring updated)** |
| `macroforecast/recipes/paper_methods.py` | API | Paper method factory | `paper_methods` | yes (C66: 1 docstring: `no-lag DAG` → `no-lag pipeline`) |
| `README.md` | Docs | Project README | — | **yes (C66: `L3 features (DAG, 37 ops)` → `L3 features (pipeline, 37 ops)`)** |
...
| `docs/tutorial/index.md` | Docs | Toctree: standalone tutorials first; recipes as advanced topic | navigation | **yes (C67: reordered, standalone-first framing)** |
| `docs/tutorial/01_first_forecast.md` | Docs | 5-min standalone: LinearAR(p=2) on synthetic AR(2) data | full tutorial | **yes (C67: full rewrite, 211 → 95 lines)** |
| `docs/tutorial/02_full_study.md` | Docs | 3-model comparison: LinearAR/PCR/FAAR + TimeSeriesSplit OOS | full tutorial | **yes (C67: full rewrite, 491 → 125 lines)** |
| `docs/tutorial/03_custom_model.md` | Docs | BaseEstimator + RegressorMixin subclass pattern (ConstantTrendPlusAR) | full tutorial | **yes (C67: full rewrite, 258 → 130 lines)** |
| `docs/tutorial/two_entry_points.md` | Docs | Standalone API = library mode (default); recipes = orchestration mode | full tutorial | **yes (C67: refreshed, standalone API leads)** |
...
## Cycle 66 — DAG Jargon Cleanup (R3-P3)

Public-facing surface (docstrings, error messages, tutorials, README) replaces "DAG" jargon with accessible vocabulary (`recipe pipeline`, `step graph`, `recipe node`). Quantitative impact: macroforecast/ DAG count 464 → 10 (98% reduction in public surface); docs/ non-archive 131 → 7. Internal orchestration vocabulary preserved in `mf.core.dag` (with new module docstring marking it as internal). Public classes (`DAG`, `DAGValidationError`, etc.) not renamed — breaking changes deferred. 1 internal file renamed: `wizard/pages/layer_dag.py` → `layer_step_graph.py`.
...
| `macroforecast/wizard/pages/layer_dag.py` | Renamed to `layer_step_graph.py`; 5 UI strings updated |
| `macroforecast/wizard/pages/layer_form.py` | Import updated; docstring updated |
...
## Cycle 67 — Tutorial 01-03 Standalone-First Rewrite (R3-P4a)
