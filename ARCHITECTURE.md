...
> Last updated: C66 (2026-05-23) — R3-P3. DAG jargon cleanup: public-surface docstrings, error messages, wizard UI, and docs/ replace "DAG" with `recipe pipeline`, `step graph`, `recipe node`. 464 → 10 in macroforecast/ (98% reduction); 131 → 7 non-archive in docs/ (95% reduction). Internal orchestration vocab in `mf.core.dag` preserved with new internal-label docstring. `wizard/pages/layer_dag.py` renamed to `layer_step_graph.py`.
