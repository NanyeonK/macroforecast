...
> Last updated: C64 (2026-05-23) — R3-P1b. 8 new model promotions (tree + neural) bring mf.models to 30 classes. 5 existing feature_selection classes refactored to sklearn BaseEstimator. 2 pre-existing runtime bugs fixed.
...
| Module / File | Layer | Purpose | Key Exports | Changed in C64 |
| --- | --- | --- | --- | --- |
...
| `macroforecast/__init__.py` | API | Package init, version declaration, lazy imports | `__version__` = `"0.9.3b1"`; `_LAZY_MODULES` extended with 4 new packages | no |
