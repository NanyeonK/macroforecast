...
| `level_feature_block` | `none`, `target_level_addback`, `x_level_addback`, `selected_level_addbacks` | Level add-backs are executable for raw-panel feature builders. Target add-back appends observed `target_t` / `target_origin`; X-level add-back appends raw-level `H` predictor values preserved before official transforms/T-codes; selected subset add-back restricts those columns via `leaf_config.selected_level_addback_columns`. Level-growth pairs remain registry-only. |
