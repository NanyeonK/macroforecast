...
| Other feature-block primitive axes | `rotation_feature_block=none` | Non-rotated feature representation is executable and records explicit no-rotation provenance when selected. `feature_block_set`, non-none `rotation_feature_block` values, and `feature_block_combination` remain registry-only. |
