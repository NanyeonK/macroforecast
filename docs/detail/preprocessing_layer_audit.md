...
| `target_normalization` | `none` | Z-score variants are contract-defined through `target_scale_contract_v1` but registry-only until normalization is fit inside each training window. |
...
| `target_normalization='zscore_train_only'` | `registry_only` | `target_scale_contract_v1` exists, but runtime does not fit normalization per training window |
| `target_normalization='robust_zscore'` | `registry_only` | `target_scale_contract_v1` exists, but runtime does not fit normalization per training window |
