...
| 2 | Research preprocessing / feature construction | Researcher-chosen target/X transforms, scaling, imputation, outlier treatment, PCA/factors, feature selection, deterministic features, custom preprocessors. |
| 3 | Forecast generator / model / benchmark | Model family, benchmark family, feature builder, forecast type, forecast object, training windows, refit, lags, tuning, model execution settings. |
...
- Layer 2 changes the representation of the official frame for a study. It owns
  extra preprocessing after official dataset transforms.
- Layer 3 generates forecasts. Benchmarks belong here because they are forecast
  generators, not data definitions.
