...
| `multi_step_raw_panel_payload_v1` | Layer 3 | `owned` | raw-panel iterated generators | artifact writer and evaluation | `operational_narrow` | Operational for `hold_last_observed`, `observed_future_x`, `scheduled_known_future_x`, and `recursive_x_model`/`ar1` raw-panel iterated point forecasts. It distinguishes step predictions, final horizon predictions, recursive target-history state, assumed, observed, scheduled, or recursively forecast future-X path, configured scheduled-known columns, recursive-X model family, and path-level metrics. |
