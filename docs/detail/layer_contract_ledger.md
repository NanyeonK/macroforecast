...
| `fred_sd_native_frequency_block_payload_v1` | Layer 2 | `consumed` | FRED-SD mixed-frequency representation builder | registered custom Layer 3 models, `midas_almon`, and `midasr_nealmon` | `operational_narrow` | Emitted when `fred_sd_mixed_frequency_representation=native_frequency_block_payload` or `mixed_frequency_model_adapter`. Groups selected FRED-SD columns by inferred native frequency and records `block_order`, `blocks`, and `column_to_native_frequency`. Requires FRED-SD data, raw-panel feature runtime, direct forecasts, and a registered custom model, `model_family=midas_almon`, or `model_family=midasr_nealmon`. |
| `fred_sd_mixed_frequency_model_adapter_v1` | Layer 3 | `owned` | registered custom mixed-frequency model adapter, `midas_almon`, or `midasr_nealmon` | artifact writer / manifest | `operational_narrow` | Adapter route over `fred_sd_native_frequency_block_payload_v1`. Current built-in slices are `midas_almon`, a ridge-regularized Almon distributed-lag direct executor, and `midasr_nealmon`, a Python restricted-NLS slice aligned to R `midasr::midas_r` with `nealmon` weights. Broader MIDAS families, regularized group MIDAS, and state-space estimators remain future. |
