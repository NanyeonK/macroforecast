...
| `fred_sd_native_frequency_block_payload_v1` | Layer 2 | `consumed` | FRED-SD mixed-frequency representation builder | registered custom Layer 3 models and `midas_almon` | `operational_narrow` | Emitted when `fred_sd_mixed_frequency_representation=native_frequency_block_payload` or `mixed_frequency_model_adapter`. Groups selected FRED-SD columns by inferred native frequency and records `block_order`, `blocks`, and `column_to_native_frequency`. Requires FRED-SD data, raw-panel feature runtime, direct forecasts, and a registered custom model or `model_family=midas_almon`. |
| `fred_sd_mixed_frequency_model_adapter_v1` | Layer 3 | `owned` | registered custom mixed-frequency model adapter or `midas_almon` | artifact writer / manifest | `operational_narrow` | Adapter route over `fred_sd_native_frequency_block_payload_v1`. Current built-in slice is `midas_almon`, a ridge-regularized Almon distributed-lag direct executor. Full nonlinear MIDAS, regularized group MIDAS, and state-space estimators remain future. |
