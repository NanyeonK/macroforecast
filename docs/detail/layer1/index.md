...
Full exposes the complete Layer 1 official-frame contract. The live registry has **18 Layer 1 axes**. The Navigator primary tree shows 16 of them; `state_selection` and `sd_variable_selection` are lower source-load selectors used by explicit FRED-SD selector helpers and group resolution.
...
| Step | Group | Axes |
|---|---|---|
| 4.1.1 | [Source and frame](source_frame.md) | `dataset`, `source_adapter`, `frequency`, `information_set_type` |
...
|---|---|---|
| `dataset` | required user choice | required |
| `source_adapter` | same as `dataset` | omitted means use the canonical loader for `dataset`; set only for `custom_csv` / `custom_parquet` |
