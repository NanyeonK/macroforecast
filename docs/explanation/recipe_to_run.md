...
that may span hundreds of forecast origins, multiple model options, and
...
Each step serves a distinct purpose, and the ordering is not incidental.
...
Errors at this step are raised immediately. The output directory is not created.
No artifacts are written. This is the design intent: a typo in an axis name,
a misspelled model option, or a reference to a future item should cost the
...
- **`step`**: executes a registered operation (a feature transform, a model
  option, an importance method) with specified configuration.
...
directories human-readable (for example, `cell_n_lag=1_model=ridge/`).
...
The per-cell seed is derived at this step. If the recipe sets
...
## Step 5: Hash Sinks and Write Manifest
...
the run did not reach Step 5, which is useful for diagnosing interrupted runs.

---

## Why This Design

Each step separation encodes a specific design choice.
...
misspells a model name discovers the mistake before any data is loaded.
