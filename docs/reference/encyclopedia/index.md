...
Encyclopedia-style browse for every macroforecast schema choice. Each layer, sub-layer, axis, and option value has its own page or anchor; the tree below is generated from the live `LayerImplementationSpec` registry plus the `OPTION_DOCS` documentation registry under `macroforecast/scaffold/option_docs/`. These pages are auto-generated — do not hand-edit them; changes to descriptions belong in the corresponding `OptionDoc` entry in `macroforecast/scaffold/option_docs/`.
...
- Option values: 631
- OptionDoc entries (full prose): 631
