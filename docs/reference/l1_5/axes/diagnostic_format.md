...
> Axis ``diagnostic_format`` on sub-layer ``L1_5_Z_export`` (layer ``l1_5``).
...
L1.5 export -- plain csv table -- one row per series / metric.
...
Diagnostic artifacts land under ``manifest.diagnostics/l1_5/`` with one file per axis per format chosen here.
...
L1.5 export -- single-file html report with embedded plots and tables.
...
L1.5 export -- machine-readable json dump of every diagnostic value.
...
L1.5 export -- latex ``tabular`` snippets ready to ``\input`` into a paper.
...
L1.5 export -- emit json + png + (optional) pdf / html in a single run.
...
L1.5 export -- vector pdf figures (matplotlib backend).
...
L1.5 export -- rasterised png figures (matplotlib backend).
