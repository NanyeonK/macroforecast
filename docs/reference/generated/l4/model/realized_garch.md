...
| `mean_model` | `str enum {"constant"}` | `'"constant"'` | — | Mean model for the return equation. Only 'constant' is supported in v0.9.3 (AR-mean deferred). Raises ValueError for any other value. |
...
Set ``params.model = "realized_garch"`` in the relevant layer to activate this op within a recipe:
