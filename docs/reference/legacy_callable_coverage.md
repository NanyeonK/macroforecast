...
| Filters and smoothers | `macroforecast.filters` | Direct one-series HP, Hamilton, Savitzky-Golay, wavelet-style, and AlbaMA filters/smoothers with metadata. |
| Feature construction | `macroforecast.feature_engineering` | Direct, average, and path targets; lags; rolling means; filter-to-feature wrappers including AlbaMA adaptive moving-average features with R-code-aligned terminal-node weights; MARX/MAF; mixed-frequency lags; time/season dummies; transforms; polynomial/interactions; PCA, group PCA, sparse PCA, varimax, PLS, SIR, DFM; random projection/Nystroem; selection methods; reusable feature specs. |
| Feature analysis | `macroforecast.feature_analysis` | Overview, correlation, target correlation, factor diagnostics, lag diagnostics, MARX diagnostics, AlbaMA/effective-window weight summaries, stage shift, selection stability/similarity, custom diagnostics. |
...
| Interpretation | `macroforecast.interpretation` | Native coefficients/importances, permutation/Strobl/LOFO, PDP/ALE/Friedman-H, SHAP variants, anatomy/PBSV forecast-accuracy attribution, forecast decomposition, cumulative contribution, group/lineage/transformation attribution, OLS attention, MRF GTVP, VAR IRF/FEVD/historical decomposition, neural attribution, LSTM hidden states, custom interpretation. |
