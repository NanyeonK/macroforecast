...
| AlbaMA terminal-node weights | `tests/feature_engineering/test_albama.py` | `feature_engineering.albama()` matches the local R script's terminal-node co-membership logic in root-leaf fixtures and `feature_analysis.recent_weight_share()` enforces one-sided no-future weights. |
