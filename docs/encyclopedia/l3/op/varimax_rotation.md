# `varimax_rotation` -- Alias for ``varimax`` -- rotation step in a multi-stage factor pipeline.
...
Set ``params.op = "varimax_rotation"`` in the relevant layer to activate this op within a recipe:
...
See also: `varimax`, `pca` (on the same axis).
