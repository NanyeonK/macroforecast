...
Docs only expose stages whose per-axis walk is finished. Stages in progress live in plan files under `plans/` and in source comments, not in user-facing docs. Layer 0 (Design) and Layer 1 (Data) are complete as of v0.9.4. Layer 2+ axes are documented only via the in-progress plan files, not in user_guide or getting_started.
...
The current canonical example is `docs/user_guide/data/source.md`. New axis pages should mirror its structure. Deviations need a one-paragraph rationale.
