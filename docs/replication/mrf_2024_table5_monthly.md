...
| Package defect surfaced | **one degenerate tree destroyed the whole forecast** — 13/100 MRF model-cells were lost; root-caused to the adapter and **fixed in PR #478** (§5) |
| Table completeness | **no missing cells** — all 275 model-cells carry a number after the fix (0 undefined) |
...
- the largest per-target gap is INF (0.060 vs 0.102); every other target moves by ≤0.004
...
absolute per-cell difference over the horizons. Every cell carries a number: the eleven
`(target, horizon)` cells that the first run lost to the defect in §5 were re-run against
the fixed code, and the numbers below are that completed set.
...
| VARRF | 1.031 / 1.02 | 1.026 / 1.08 | 0.993 / 1.03 | 0.963 / 0.96 | 0.840 / 0.89 | 0.031 |

**IP overall mean\|Δ\| = 0.082**
...
| ARRF | 0.984 / 0.91 | 0.857 / 0.89 | 0.816 / 0.97 | 0.794 / 0.99 | 0.677 / 0.91 | 0.138 |
| FA-ARRF | 0.958 / 0.90 | 0.877 / 0.82 | 0.772 / 0.98 | 0.778 / 0.94 | 0.685 / 0.97 | 0.154 |
| Tiny ARRF | 1.023 / 0.98 | 1.129 / 1.03 | 1.257 / 1.16 | 1.196 / 1.17 | 1.144 / 1.28 | 0.081 |
...
| ARRF | 0.955 / 0.95 | 0.887 / 0.90 | 0.718 / 0.72 | 0.703 / 0.73 | 0.705 / 0.71 | 0.010 |
| FA-ARRF | 0.948 / 0.96 | 0.843 / 0.88 | 0.722 / 0.82 | 0.679 / 0.67 | 0.679 / 0.69 | 0.033 |
...
| ARRF | 1.053 / 0.99 | 0.932 / 1.06 | 0.664 / 0.70 | 0.606 / 0.68 | 0.549 / 0.69 | 0.089 |
| FA-ARRF | 0.983 / 0.98 | 0.928 / 0.85 | 0.625 / 0.62 | 0.555 / 0.65 | 0.558 / 0.63 | 0.051 |
| Tiny ARRF | 0.942 / 0.96 | 0.915 / 1.00 | 0.923 / 1.07 | 0.984 / 1.07 | 0.992 / 0.90 | 0.085 |
| VARRF | 0.939 / 0.93 | 0.811 / 0.88 | 0.606 / 0.67 | 0.520 / 0.64 | 0.502 / 0.70 | 0.092 |

**SPREAD overall mean\|Δ\| = 0.186**
...
| ARRF | 1.028 / 1.01 | 1.049 / 1.04 | 1.021 / 1.02 | 1.003 / 1.00 | 0.854 / 1.00 | 0.035 |
...
**HOUST overall mean\|Δ\| = 0.057**

### Family accuracy (mean\|Δ\| by model family, vs AR(12))

| target | MRF | RF/tree | linear | AO |
|---|---|---|---|---|
| IP | 0.035 | 0.060 | 0.032 | 0.258 |
| UR | 0.123 | 0.108 | 0.039 | 0.127 |
| INF | 0.028 | 0.110 | 0.025 | 0.086 |
| SPREAD | 0.079 | 0.342 | 0.099 | 0.251 |
| HOUST | 0.064 | 0.031 | 0.090 | 0.050 |
...
|---|---|---|---|---|---|
| IP | **0.987** | 1.119 | **0.980** | 1.035 | **reproduced** |
| UR | **0.832** | 1.070 | **0.914** | 1.038 | **reproduced** |
| INF | **0.845** | 0.911 | **0.886** | 0.974 | **reproduced** |
| SPREAD | **0.706** | 1.022 | **0.776** | 1.320 | **reproduced** |
| HOUST | **0.999** | 1.075 | **1.030** | 1.106 | **reproduced** |
...
the paper's strongest monthly claim reproduces almost exactly (`ARRF` mean\|Δ\| = 0.010).

---

## 5. Package defect found and fixed (objective 2): one degenerate tree destroyed the forecast

The first run of this table lost **13 of 100 MRF model-cells** and **0 of 175 non-MRF
cells**: the MRF returned `nan` for at least one out-of-sample month, and a single `nan`
propagated through the RMSE to destroy the whole cell. `Tiny ARRF` lost all five UR
horizons.

### Root cause — the adapter, not the vendored MRF

`output["pred_ensemble"]` is the **raw per-tree committee** `(B, n_oos)`, so reducing it
*is* the ensemble average — and the adapter reduced it with a **nan-propagating**
`arr.mean(axis=0)`. The vendored backend does not do that: its own ensemble output
`output["pred"]` is `pd.DataFrame(committee).mean(axis=0)`, which is **nan-skipping**, as
is the R prototype. The adapter ignored the backend answer and recomputed the average
under different semantics, so **one degenerate tree out of B wiped out the forecast for
that row** — and with it the RMSE, and the cell.
...
suppresses the backend `invalid value encountered in divide` RuntimeWarning, which is
the signal that would have exposed it.
...
2. `degenerate_tree_predictions_` is exposed and a warning raised when trees are skipped,
   so the condition is visible instead of silent;
3. the committee axis is resolved from the known tree count. Deducing it from shape alone
   silently averaged **across forecast dates instead of across trees** whenever
   `B == n_oos`, and `B=25` is the package default, so that collision is reachable.

### Statistically identical — measured twice (objective 4)

| check | result |
|---|---|
| 395 random NaN-free committees, unit level | **bit-identical**, max\|Δ\| = 0.000e+00 |
| **MRF cells re-run on real data** | **56 identical, 0 changed, max\|Δ\| = 0.000e+00** |
| non-MRF cells in the same re-runs | 128 identical, 12 differ by exactly **1 ULP** |
| lost-to-recovered | **24 entries recovered, 0 finite values lost** |

The MRF path — the only path the fix touches — is **exactly unchanged wherever it already
produced a number**; the fix only turns `nan` into a value. The 1-ULP differences are in
`RF`, `RF-MAF` and `AR+RF`, which are scikit-learn fits the change does not touch:
scikit-learn reduces tree predictions across threads, and that summation order is not
bit-reproducible across processes. `[note]` This bounds the reproducibility of the
scikit-learn rows of this table at ~1 ULP rather than bit-exact; the MRF rows are exact.
...
| **fixed** | MRF `nan` forecasts (§5) — 13/100 MRF cells lost in the first run; root cause was the adapter nan-propagating committee reduction, **fixed in PR #478**, all cells re-run and recovered. |
| `[note]` | scikit-learn rows reproduce to ~1 ULP across processes, not bit-exact (threaded reduction); MRF rows are bit-exact. |
...
each, ~2.4 h per cell, 24.6 h for the full table plus 10.3 h to re-run the eleven cells
the defect had cost).
