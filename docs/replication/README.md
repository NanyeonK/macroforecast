...
| Han, Lu & Zhou, macro financial trends and market expected returns | Table 3 — dense trend combinations; `R²_OS` vs the historical average | The authors' own 696-month forecast paths are reproduced to machine precision (max\|Δ\| ≤ 7.8e-16) and their published `R²_OS` exactly, once the forecast origin is aligned; the widest two ladders are unrun on cost grounds. | STRONG | STRONG | STRONG | STRONG | `han_lu_zhou_2025.md` |
