...
false-positive rate. The v0.9.4 fix removed the always-accept-one fallback,
...
| High false-positive rate | `n_shadow_copies` set too low | Keep the default `n_shadow_copies=6` or higher, since v0.9.4 verified that values below 6 inflate the FP rate |
