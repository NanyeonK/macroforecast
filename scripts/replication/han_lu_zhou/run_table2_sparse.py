...
DESIGNS = sys.argv[2].split(",") if len(sys.argv) > 2 else ["ma6", "ma12", "lag6", "lag12"]
...
SIGNALS = {
    ...
    "lag6":    lambda: {k: v for b in range(1, 7) for k, v in lag_block(b).items()},
    "lag12":   lambda: {k: v for b in range(1, 13) for k, v in lag_block(b).items()},
    "ma6":     lambda: {k: v for L in (1, 3, 6) for k, v in ma_block(L).items()},
    "ma12":    lambda: {k: v for L in (1, 3, 6, 12) for k, v in ma_block(L).items()},
}
# authors' Table 2 values, derived from their archived 696-month forecast paths
# printed Table 2 of the published paper (RAPS), verified against the PDF
TARGET = {"current": 0.60, "lag6": 0.59, "ma6": 0.70, "lag12": 0.57, "ma12": 0.76}
LABEL = {"current": "A: Xt (14)", "lag6": "B: Xt..Xt-5 (84)",
         "ma6": "B: Xt;MA3;MA6 (42)", "lag12": "C: Xt..Xt-11 (168)",
         "ma12": "C: Xt;MA3;MA6;MA12 (56)"}
