...
def test_l7_multi_method_parses():
    assert (
        validate_layer(
            parse_layer_yaml(_example("l7_multi_method.yaml"), "l7")
        ).has_hard_errors
        is False
    )
...
def test_l7_coulombe_groups_parses():
    assert (
        validate_layer(
            parse_layer_yaml(_example("l7_coulombe_groups.yaml"), "l7")
        ).has_hard_errors
        is False
    )
...
def test_l7_transformation_attribution_parses():
    assert (
        validate_layer(
            parse_layer_yaml(_example("l7_transformation_attribution.yaml"), "l7")
        ).has_hard_errors
        is False
    )
...
def test_l7_temporal_parses():
    assert (
        validate_layer(
            parse_layer_yaml(_example("l7_temporal.yaml"), "l7")
        ).has_hard_errors
        is False
    )
...
def test_l7_shap_tree_rejects_linear_model():
    assert validate_layer(
        parse_layer_yaml(make_l7_yaml(op="shap_tree", model_family="ridge"), "l7")
    ).has_hard_errors


def test_l7_shap_linear_rejects_tree_model():
    assert validate_layer(
        parse_layer_yaml(make_l7_yaml(op="shap_linear", model_family="xgboost"), "l7")
    ).has_hard_errors


def test_l7_shap_deep_requires_nn_model():
    assert validate_layer(
        parse_layer_yaml(make_l7_yaml(op="shap_deep", model_family="ridge"), "l7")
    ).has_hard_errors
...
def test_l7_shap_kernel_works_on_any_model():
    assert (
        validate_layer(
            parse_layer_yaml(
                make_l7_yaml(op="shap_kernel", model_family="xgboost"), "l7"
            )
        ).has_hard_errors
        is False
    )
...
def test_l7_shap_kernel_works_on_linear():
    assert (
        validate_layer(
            parse_layer_yaml(make_l7_yaml(op="shap_kernel", model_family="ridge"), "l7")
        ).has_hard_errors
        is False
    )
...
def test_l7_mrf_gtvp_only_for_mrf():
    assert validate_layer(
        parse_layer_yaml(make_l7_yaml(op="mrf_gtvp", model_family="xgboost"), "l7")
    ).has_hard_errors
...
def test_l7_gradient_methods_reject_tree_model():
    for op in ["integrated_gradients", "saliency_map", "deep_lift", "gradient_shap"]:
        assert validate_layer(
            parse_layer_yaml(make_l7_yaml(op=op, model_family="xgboost"), "l7")
        ).has_hard_errors
...
def test_l7_var_specific_ops_reject_non_var():
    ...
    for op in ["fevd", "historical_decomposition", "orthogonalised_irf"]:
        assert validate_layer(
            parse_layer_yaml(make_l7_yaml(op=op, model_family="ridge"), "l7")
        ).has_hard_errors
...
def test_l7_lasso_inclusion_rejects_xgboost():
    assert validate_layer(
        parse_layer_yaml(
            make_l7_yaml(op="lasso_inclusion_frequency", model_family="xgboost"), "l7"
        )
    ).has_hard_errors
...
def test_l7_bvar_pip_rejects_non_bvar():
    assert validate_layer(
        parse_layer_yaml(make_l7_yaml(op="bvar_pip", model_family="var"), "l7")
    ).has_hard_errors
...
def test_l7_user_groups_works_in_leaf_config():
    ...
    user_groups = resolve_axes(normalize_to_dag_form(layer, "l7"))["leaf_config"][
        "user_groups"
    ]
...
def test_l7_lineage_attribution_requires_l3_metadata():
    ...
    sink_refs = [
        source["selector"]["sink_name"]
        for source in layer["nodes"]
        if source.get("type") == "source"
    ]
    assert "l3_metadata_v1" in sink_refs


def test_l7_attention_weights_operational_after_b10_promotion():
    # Phase B-10 paper-10 promotion: ``attention_weights`` carries the
    # paper-faithful Eq. 3 closed-form matrix
    # ``Omega = X_test (X'_train X_train)^{-1} X'_train``. The validator
    # must now accept the op (it was hard-rejected as ``future`` in v0.1
    # through v0.9.0). The op is paired here with ``ols`` because Eq. 3
    # is defined for linear families; non-linear families would route to
    # paper §5 nonlinear Attention Regression which is out of scope.
    layer = parse_layer_yaml(
        make_l7_yaml(op="attention_weights", model_family="ols"), "l7"
    )
...
def test_l7_mrf_gtvp_operational_after_coefficient_path_landing():
    ...
    layer = parse_layer_yaml(
        make_l7_yaml(op="mrf_gtvp", model_family="macroeconomic_random_forest"), "l7"
    )
    report = validate_layer(layer)
    assert not report.has_hard_errors
...
def test_l7_default_figure_mapping_for_transformation_attribution():
    assert (
        get_op("transformation_attribution").default_figure_type == "shapley_waterfall"
    )
...
def test_l7_top_k_features_default():
    layer = parse_layer_yaml("7_interpretation:\n  enabled: true\n  nodes: []\n", "l7")
    assert (
        resolve_axes(normalize_to_dag_form(layer, "l7"))["top_k_features_to_show"] == 20
    )
...
def test_l7_transformation_attribution_sink_empty_when_step_unused():
    ...
    assert (
        "l7_transformation_attribution_v1" not in sinks
        or sinks["l7_transformation_attribution_v1"] is None
    )
...
def test_l7_operational_ops_registered_after_honesty_pass():
    ...
    operational = [
        ...
        if "l7" in op.layer_scope and op.status == "operational"
    ]
