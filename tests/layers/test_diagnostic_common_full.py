...
def _module(layer_id: str):
    if layer_id == "l3_5":
        ...
        from macroforecast.diagnostics.generator import schema as module  # type: ignore[assignment]
    return module
...
def test_l8_saved_objects_all_diagnostics_options_present():
    ...
    saved_objects = {
        ...
        "model_artifacts", "combination_weights", "feature_metadata", "clean_panel", "raw_panel",
        "diagnostics_l3_5", "diagnostics_l4_5", "diagnostics_all", "tests", "importance",
        "transformation_attribution",
    }
    assert op_spec.layer_scope == ("l8",)
    saved_object_names = {
        "l3_5": "diagnostics_l3_5",
        "l4_5": "diagnostics_l4_5",
    }
    for layer_id in ["l3_5", "l4_5"]:
        assert saved_object_names[layer_id] in saved_objects
