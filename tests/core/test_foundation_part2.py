...
def test_execute_node_uses_source_context_and_cache(tmp_path: Path, mock_clean_panel) -> None:
    ...
    result = execute_node(dag.nodes["lag_x"], dag, {"sources": {"preprocessing.preprocessed_panel_v1": mock_clean_panel}}, tmp_path)
    cached = execute_node(dag.nodes["lag_x"], dag, {"sources": {"preprocessing.preprocessed_panel_v1": mock_clean_panel}}, tmp_path)
...
3_feature_engineering:
  nodes:
    - id: src_clean
      type: source
      selector: {layer_ref: preprocessing, sink_name: preprocessed_panel_v1, subset: {predictors: true}}
    - id: lag_x
      type: step
      op: lag
