...
def _patch_infrastructure(monkeypatch, fake_df, fake_tcodes):
    ...
    import macroforecast.data.sources.fred_qd as fqd
    ...
    monkeypatch.setattr(
        "macroforecast.data.sources.fred_qd.normalize_version_request",
        lambda *a, **kw: fake_req,
    )
    monkeypatch.setattr(
        "macroforecast.data.sources.fred_qd.get_raw_file_path",
        lambda *a, **kw: fake_target,
    )
    monkeypatch.setattr(
        "macroforecast.data.sources.fred_qd.parse_fred_csv",
        lambda *a, **kw: (fake_df, fake_tcodes),
    )
    monkeypatch.setattr(
        "macroforecast.data.sources.fred_qd.build_raw_artifact_record",
        lambda *a, **kw: _make_fake_artifact(),
    )
    monkeypatch.setattr(
        "macroforecast.data.sources.fred_qd.append_raw_manifest_entry",
        lambda *a, **kw: None,
    )
...
def test_nat_safe_data_through_no_nat(monkeypatch):
    ...
    from macroforecast.data.sources.fred_qd import load_fred_qd
    result = load_fred_qd()
