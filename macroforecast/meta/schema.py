...
def configure(
    *,
    ...
    reproducibility_policy: ReproducibilityMode = "seeded_reproducible",
    compute_policy: ComputeMode = "serial",
    random_seed: int | None | object = _MISSING,
    parallel_unit: ParallelUnit | None = None,
    n_workers: int | Literal["auto"] | None = None,
    gpu_deterministic: bool | None = None,
    **leaf_config: Any,
) -> dict[str, dict[str, Any]]:
    """Return the canonical ``0_meta`` recipe block.

    This is the callable authoring surface for L0/meta. It does not execute a
    study; it creates the same recipe block that YAML authoring would provide,
    then validates that block through the L0 schema validator.
    """
    ...
    return {L0_BLOCK_KEY: block}


def l0(**kwargs: Any) -> dict[str, dict[str, Any]]:
    """Alias for :func:`configure` for layer-numbered callable authoring."""
...
def build_layer_block(
    ...
    **leaf_config: Any,
    ...
    """Build a canonical L0 layer body without the top-level ``0_meta`` key."""
    ...
    leaf: dict[str, Any] = dict(leaf_config)

    if random_seed is _MISSING:
        if reproducibility_policy == "seeded_reproducible":
            leaf["random_seed"] = DEFAULT_RANDOM_SEED
    elif random_seed is not None:
        leaf["random_seed"] = int(cast(int, random_seed))

    if compute_policy == "parallel" and parallel_unit is None:
        parallel_unit = "cells"
    if parallel_unit is not None:
        leaf["parallel_unit"] = parallel_unit
    if n_workers is not None:
        leaf["n_workers"] = n_workers
    if gpu_deterministic is not None:
        leaf["gpu_deterministic"] = bool(gpu_deterministic)
...
def resolve_axes(dag: DAG, recipe_context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if resolved["reproducibility_policy"] == "seeded_reproducible" and resolved["random_seed"] is None:
        resolved["random_seed"] = DEFAULT_RANDOM_SEED
    return resolved
...
def validate_layer(layer: Any | dict[str, Any] | str) -> Any:
    ...
    if reproducibility_policy == "seeded_reproducible":
        random_seed = leaf_config.get("random_seed", DEFAULT_RANDOM_SEED)
...
def _raw_layer(layer: Any) -> dict[str, Any]:
...
def _report(location: str, message: str) -> Any:
    from macroforecast.core.validator import ValidationReport
