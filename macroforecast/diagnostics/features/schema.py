...
OPTIONS = {
    ...
    "selection_view": {"none", "selected_list", "selection_count_per_origin", "selection_stability", "multi"},  # "none" disables selection diagnostics without error (v0.8.x)
...
def _validate_values(resolved: L3_5ResolvedAxes, fixed: dict[str, Any], context: dict[str, Any]) -> list[Any]:
    ...
    if "selection_view" in fixed and fixed.get("selection_view") not in {"none", None} and not context.get("has_feature_selection_step", False):  # selection_view=none is a no-op (v0.8.x)
