...
class MacroRandomForest:
    ...
    def _name_translations(self):
        ...
        # numpy 2.x compat: np.where rejects 0-d boolean inputs; coerce S_pos
        # to a 1-D array so the equality always returns a 1-D mask.
    ...
    def _input_safety_checks(self):
        ...
        if self.y_pos in list(self.x_pos):
            ...
            # numpy 2.x compat: see _name_translations note.
            self.trend_pos = np.where(
                np.asarray(self.x_pos) == self.trend_pos)  # foolproof 1
    ...
    def _ensemble_loop(self):
        ...
        # pandas 2.x compat: ``set_index`` treats list inputs as column
        # selectors, not as the new index values. Assign a pd.Index
        # explicitly so the oos row positions become the frame's index.
        ...
        for i in range(0, len(leafs)):
            ...
            if len(ind) > 0:
                ...
                if len(ind_all) == 1:
                    ...
                    # numpy 2.x compat: see len(ind_all)==1 branch.
                    fitted_vals = np.asarray(
                        zz_all @ ((1-self.HRW) * beta_hat + self.HRW*b0)
                    ).ravel()
