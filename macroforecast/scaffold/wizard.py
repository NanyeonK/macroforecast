...
Entry point: ``python -m macroforecast scaffold`` (see ``cli.py``).
"""
...
def _show_option_help(axis: introspect.AxisInfo, target: str, options: Iterable[introspect.OptionInfo]) -> None:
    ...
    if doc is None:
        ...
        if doc.last_reviewed:
            _print()
            _print(f"Last reviewed: {doc.last_reviewed} by {doc.reviewer or 'macroforecast author'}")
    _print(_RULE)
...
def _intro(input_fn: Callable[[str], str]) -> str:
    _heading("macroforecast — recipe scaffold wizard")
    _print()
...
_LAYER_KEYS: dict[str, str] = {
    ...
    # ``macroforecast.core.layers.l{0..8}`` and ``l{1,2,3,4}_5``.
    "l0": "0_meta",
    "l1": "1_data",
