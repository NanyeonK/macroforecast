...
MODEL_SPECS: dict[str, ModelSpec] = {
    ...
    "slow_growing_tree": _spec(
        ...
        parameters=(
            ...
            _p("eta_depth_step", 0.01, "float", "Per-depth increase in soft split leakage.", False),
            _p("eta_max_plateau", 0.5, "float", "Upper plateau for depth-adjusted leakage.", False),
            _p("mtry_frac", 1.0, "float", "Fraction of candidate features considered at each split."),
            _p("max_depth", 10, "int | None", "Maximum tree depth."),
            _p("min_leaf_size", 5, "int", "Minimum effective leaf size."),
            _p("random_state", 0, "int", "Tree random seed.", False),
        ),
        spaces={
            "small": {"eta": (0.05, 0.1), "herfindahl_threshold": (0.2, 0.3), "mtry_frac": (0.75, 1.0), "max_depth": (5, 10), "min_leaf_size": (3, 5)},
            "standard": {"eta": (0.03, 0.05, 0.1), "herfindahl_threshold": (0.15, 0.25, 0.35), "mtry_frac": (0.5, 0.75, 1.0), "max_depth": (5, 10, None), "min_leaf_size": (3, 5, 10)},
            "wide": {"eta": (0.01, 0.03, 0.05, 0.1, 0.2), "herfindahl_threshold": (0.1, 0.15, 0.25, 0.35, 0.5), "mtry_frac": (0.33, 0.5, 0.75, 1.0), "max_depth": (3, 5, 10, 20, None), "min_leaf_size": (2, 3, 5, 10)},
        },
        method="random",
    ...
    "bagging": _spec(
        ...
        spaces={
            "small": {"base": ("ridge", "lasso"), "n_estimators": (10, 25), "max_samples": (0.6, 0.8), "strategy": ("standard",), "block_length": (4,)},
            "standard": {"base": ("ridge", "lasso", "decision_tree"), "n_estimators": (25, 50, 100), "max_samples": (0.5, 0.7, 0.9), "strategy": ("standard", "block"), "block_length": (4, 8)},
            "wide": {"base": ("ridge", "lasso", "elastic_net", "decision_tree", "random_forest"), "n_estimators": (25, 50, 100, 200), "max_samples": (0.4, 0.6, 0.8, 1.0), "strategy": ("standard", "block"), "block_length": (2, 4, 8, 12)},
        },
        method="random",
    ...
    "booging": _spec(
        ...
        parameters=(
            ...
            _p("da_noise_frac", 1.0 / 3.0, "float", "Scale of feature-noise augmentation."),
            _p("da_drop_rate", 0.2, "float", "Share of augmented columns dropped per model."),
            _p("random_state", 0, "int", "Ensemble random seed.", False),
        ),
        spaces={
            "small": {"B": (5, 10), "sample_frac": (0.6, 0.8), "inner_n_estimators": (100, 300), "inner_learning_rate": (0.05, 0.1), "inner_max_depth": (2, 3), "inner_subsample": (0.5, 0.75), "da_noise_frac": (0.25, 1.0 / 3.0), "da_drop_rate": (0.1, 0.2)},
            "standard": {"B": (10, 25, 50), "sample_frac": (0.5, 0.75, 0.9), "inner_n_estimators": (300, 750, 1500), "inner_learning_rate": (0.03, 0.05, 0.1), "inner_max_depth": (2, 3, 5), "inner_subsample": (0.5, 0.75, 1.0), "da_noise_frac": (0.1, 0.25, 1.0 / 3.0), "da_drop_rate": (0.1, 0.2, 0.4)},
            "wide": {"B": (10, 25, 50, 100), "sample_frac": (0.4, 0.6, 0.75, 0.9), "inner_n_estimators": (300, 750, 1500, 2500), "inner_learning_rate": (0.01, 0.03, 0.05, 0.1), "inner_max_depth": (2, 3, 5, 8), "inner_subsample": (0.4, 0.5, 0.75, 1.0), "da_noise_frac": (0.05, 0.1, 0.25, 1.0 / 3.0, 0.5), "da_drop_rate": (0.0, 0.1, 0.2, 0.4, 0.6)},
        },
        method="random",
    ...
    "macro_random_forest": _spec(
        ...
        parameters=(
            _p("x_columns", None, "list[str] | None", "Predictors in the time-varying linear equation.", False),
            _p("S_columns", None, "list[str] | None", "State variables entering the forest split function.", False),
            _p("x_pos", None, "list[int] | None", "Reference-package predictor positions after the target column.", False),
            _p("S_pos", None, "list[int] | None", "Reference-package state positions after the target column.", False),
            ...
            _p("mtry_frac", 1.0 / 3.0, "float", "Fraction of state variables considered at each split."),
            _p("min_leaf_frac_of_x", 1.0, "float", "Minimum leaf-size multiplier relative to local x dimension."),
            _p("VI", False, "bool", "Enable variable-importance split search mode.", False),
            _p("ERT", False, "bool", "Enable extremely randomized tree split mode.", False),
            _p("quantile_rate", None, "float | None", "Optional quantile rate for quantile-oriented output.", False),
            _p("S_priority_vec", None, "list[float] | None", "Optional priority weights over state variables.", False),
            _p("random_x", False, "bool", "Use random subsets of local-linear predictors.", False),
            _p("trend_push", 1, "int", "Reference-package trend-push option.", False),
            _p("howmany_random_x", 1, "int", "Number of random local-linear predictor draws.", False),
            _p("howmany_keep_best_VI", 20, "int", "Number of best VI candidates retained.", False),
            _p("cheap_look_at_GTVPs", True, "bool", "Use the reference package's cheaper GTVP inspection.", False),
            _p("prior_var", None, "list[float] | None", "Optional prior variances for local coefficients.", False),
            _p("prior_mean", None, "list[float] | None", "Optional prior means for local coefficients.", False),
            _p("subsampling_rate", 0.75, "float", "Subsample share used by each tree."),
            _p("rw_regul", 0.75, "float", "Random-walk shrinkage strength."),
            _p("keep_forest", False, "bool", "Keep full reference forest object in memory.", False),
            _p("block_size", 12, "int", "Reference-package block size for time-series resampling.", False),
            _p("fast_rw", True, "bool", "Use fast random-walk regularization path.", False),
            _p("ridge_lambda", 0.1, "float", "Ridge penalty for local linear fits."),
            _p("HRW", 0, "int", "Reference-package hierarchical random-walk option.", False),
            _p("resampling_opt", 2, "int", "Reference MRF resampling option."),
            _p("parallelise", False, "bool", "Run the reference implementation in parallel.", False),
            _p("n_cores", 1, "int", "Reference implementation worker count.", False),
            _p("print_b", False, "bool", "Print tree progress.", False),
        ),
        spaces={
            "small": {"B": (10, 25), "minsize": (5, 10), "mtry_frac": (0.5, 1.0), "min_leaf_frac_of_x": (1.0,), "subsampling_rate": (0.75,), "rw_regul": (0.5, 0.75), "ridge_lambda": (0.1, 0.5), "resampling_opt": (2,)},
            "standard": {"B": (25, 50, 100), "minsize": (5, 10, 20), "mtry_frac": (1.0 / 3.0, 0.5, 1.0), "min_leaf_frac_of_x": (0.5, 1.0), "subsampling_rate": (0.5, 0.75), "rw_regul": (0.5, 0.75, 0.9), "ridge_lambda": (0.1, 0.5, 1.0), "resampling_opt": (1, 2)},
            "wide": {"B": (50, 100, 250), "minsize": (5, 10, 20, 40), "mtry_frac": (0.25, 1.0 / 3.0, 0.5, 0.75, 1.0), "min_leaf_frac_of_x": (0.25, 0.5, 1.0, 2.0), "subsampling_rate": (0.5, 0.63, 0.75, 0.9), "rw_regul": (0.25, 0.5, 0.75, 0.9), "ridge_lambda": (0.01, 0.1, 0.5, 1.0), "resampling_opt": (1, 2)},
        },
        method="random",
