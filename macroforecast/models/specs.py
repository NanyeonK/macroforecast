...
MODEL_SPECS: dict[str, ModelSpec] = {
    ...
    "glmboost": _spec(
        ...
        "linear",
        glmboost,
        default_params={"n_iter": 100, "learning_rate": 0.1, "center": True},
        parameters=(
    ...
    "var": _spec(
        ...
        "timeseries",
        var,
        default_params={"target": None, "n_lag": 1, "type": "const", "season": None},
        parameters=(
            _p("target", None, "str | None", "Target column in the panel.", False),
            _p("n_lag", 1, "int", "VAR lag order."),
            _p("type", "const", "str", "R vars::VAR deterministic terms: const, trend, both, or none.", False),
            _p("season", None, "int | None", "Optional centered seasonal dummies, matching vars::VAR(season=...).", False),
        ),
        spaces=_AR_SPACES,
        input_kind="panel",
    ...
    "bvar_minnesota": _spec(
        ...
        parameters=(
            _p("target", None, "str | None", "Target column in the panel.", False),
            _p("n_lag", 1, "int", "VAR lag order."),
            _p("kappa0", 2.0, "float", "FAVAR/bvartools Minnesota own-lag prior scale."),
            _p("kappa1", 0.5, "float", "FAVAR/bvartools Minnesota lag-decay exponent."),
            _p("nu0", 0.0, "float", "Inverse-Wishart degrees-of-freedom prior parameter.", False),
            _p("s0", 0.0, "float | matrix | None", "Inverse-Wishart scale prior parameter.", False),
            _p("iter", 10000, "int", "Total Gibbs iterations.", False),
            _p("burnin", 5000, "int", "Burn-in iterations discarded from posterior summaries.", False),
            _p("random_state", 0, "int", "Random seed for posterior draws.", False),
        ),
        spaces={
            "small": {"n_lag": (1, 2), "kappa0": (1.0, 2.0), "kappa1": (0.5,)},
            "standard": {"n_lag": (1, 2, 4), "kappa0": (0.5, 1.0, 2.0), "kappa1": (0.5, 1.0)},
            "wide": {"n_lag": (1, 2, 4, 6, 12), "kappa0": (0.25, 0.5, 1.0, 2.0), "kappa1": (0.5, 1.0, 2.0)},
        },
        input_kind="panel",
    ...
    "bvar_normal_inverse_wishart": _spec(
        ...
        parameters=(
            ...
            _p("vb0", 0.0, "float", "Normal prior variance scale for VAR coefficients.", False),
            _p("nu0", 0.0, "float", "Inverse-Wishart degrees-of-freedom prior parameter.", False),
            _p("s0", 0.0, "float | matrix | None", "Inverse-Wishart scale prior parameter.", False),
            _p("iter", 10000, "int", "Total Gibbs iterations.", False),
            _p("burnin", 5000, "int", "Burn-in iterations discarded from posterior summaries.", False),
    ...
    "midas_almon": _spec(
        ...
        parameters=(
            ...
            _p("theta", None, "tuple[float, ...] | None", "midasr::nealmon shape coefficients; length must equal polynomial_order.", False),
    ...
    "midas_beta": _spec(
        ...
        parameters=(
            _p("beta_params", (1.0, 1.0), "tuple[float, float]", "midasr::nbetaMT beta lag-weight shape parameters."),
            _p("alpha", 0.0, "float", "Optional ridge penalty on aggregated MIDAS regressors."),
            _p("fit_intercept", True, "bool", "Whether to fit an intercept.", False),
        ),
        spaces={
    ...
    "midas_step": _spec(
        ...
        "mixed_frequency",
        midas_step,
        default_params={"n_steps": 3, "step_bounds": None, "step_weights": None, "alpha": 0.0, "fit_intercept": True},
        parameters=(
            _p("n_steps", 3, "int", "Number of lag buckets when step_bounds is not supplied."),
            _p("step_bounds", None, "tuple[int, ...] | None", "Optional midasr::polystep-style interior cut points.", False),
            _p("step_weights", None, "tuple[float, ...] | None", "Optional raw height for each step bucket.", False),
            _p("alpha", 0.0, "float", "Optional ridge penalty on aggregated MIDAS regressors."),
            _p("fit_intercept", True, "bool", "Whether to fit an intercept.", False),
        ),
        spaces={
    ...
    "restricted_midas": _spec(
        ...
        parameters=(
            _p("weighting", "almon", "str", "Restriction map: 'almon'/'nealmon', 'beta'/'nbetaMT', or 'step'/'polystep'.", False),
            _p("polynomial_order", 2, "int", "Almon polynomial order after the aggregate scale parameter.", False),
            _p("start_params", None, "Mapping[str, Sequence[float]] | Sequence[float] | None", "midasr::midas_r-style starting values for each lag group.", False),
            _p("n_steps", 3, "int", "Number of step buckets when weighting='step' and step_bounds is not supplied.", False),
            _p("step_bounds", None, "tuple[int, ...] | None", "Optional midasr::polystep-style interior cut points.", False),
            _p("fit_intercept", True, "bool", "Whether to estimate an intercept outside the restricted lag maps.", False),
            _p("maxiter", 1000, "int", "Maximum SciPy least_squares function evaluations.", False),
        ...
        spaces={
            ...
            "wide": {"weighting": ("almon", "beta", "step"), "polynomial_order": (1, 2, 3, 4), "n_steps": (2, 3, 4)},
        },
    ...
    "favar": _spec(
        ...
        parameters=(
            ...
            _p("fctmethod", "BBE", "str", "FAVAR factor identification method: BBE or BGM.", False),
            _p("slowcode", None, "Sequence[bool] | None", "Slow-variable mask required by BBE.", False),
            _p("factorprior", None, "Mapping[str, Any] | None", "Factor loading prior controls.", False),
            _p("varprior", None, "Mapping[str, Any] | None", "BVAR prior controls for the factor VAR block.", False),
            _p("nburn", 5000, "int", "Burn-in iterations for loading/BVAR posterior draws.", False),
            _p("nrep", 15000, "int", "Saved loading draws and post-burn BVAR draw count.", False),
            _p("standardize", True, "bool", "Use R scale() semantics for X and y before factor extraction.", False),
            _p("random_state", 0, "int", "Random seed for posterior draws.", False),
        ),
        spaces={
            key: {**space, **_AR_SPACES[key]} for key, space in _FACTOR_SPACES.items()
