...
def resolve_axes(dag: DAG) -> L4ResolvedAxes:
    ...
    for node in dag.nodes.values():
        ...
        values["_active"]["validation_method"] = (
            values.get("search_algorithm") != "none"
        )
        break
...
def validate_layer(
    ...
    issues.extend(
        Issue(
            "l4_dag",
            Severity.HARD if issue.severity == "hard" else Severity.SOFT,
            "dag",
            issue.location,
            issue.message,
        )
...
def validate_recipe(recipe_yaml: dict[str, Any] | str) -> ValidationReport:
    ...
    return validate_layer(
        root["4_forecasting_model"], recipe_context=_recipe_context(root)
    )
...
def execute_layer(layer: dict[str, Any] | str):
    ...
    Wraps :func:`macroforecast.core.runtime.materialize_l4_minimal` for callers
    ...
    l3_features, _ = materialize_l3_minimal(root, l1_artifact, preprocessed_artifact)
...
def make_l4_yaml_with_validation_method(method: str) -> str:
    return (
        make_l4_yaml("ridge", search_algorithm="grid_search", validation_method=method)
...
def make_l4_yaml_with_combine_temporal(rule: str) -> str:
    return _ensemble_yaml(
        combine_op="weighted_average_forecast",
        combine_params={"weights_method": "dmsfe", "temporal_rule": rule},
        n_inputs=2,
    )
...
def make_l4_yaml_with_combine(n_inputs: int) -> str:
    return _ensemble_yaml(
        combine_op="weighted_average_forecast",
        combine_params={"weights_method": "dmsfe"},
        n_inputs=n_inputs,
    )
...
def _validate_sinks(dag: DAG) -> list[Issue]:
    ...
    return [
        _issue("l4.sinks", f"missing required sink {sink}") for sink in sorted(missing)
    ]
...
def _is_user_registered_family(family: str) -> bool:
    """Allow ``macroforecast.custom.register_model`` registered names through
...
def _validate_fit_nodes(raw: dict[str, Any]) -> list[Issue]:
    from macroforecast.models.ops import MODEL_FAMILY_STATUS  # lazy: avoids circular import at module load
    ...
    for node in raw.get("nodes", ()):
        ...
        if MODEL_FAMILY_STATUS.get(family) == "future":
            issues.append(
                _issue(
                    f"l4.{node['id']}",
                    f"model family {family} is future and not executable",
                )
            )
        elif family not in MODEL_FAMILY_STATUS and not _is_user_registered_family(
            family
        ):
            issues.append(
        ...
        if search == "cv_path" and family not in {"lasso", "ridge", "elastic_net"}:
            issues.append(
                _issue(
                    f"l4.{node['id']}",
                    "cv_path search_algorithm requires lasso, ridge, or elastic_net",
                )
            )
        if search == "grid_search" and "tuning_grid" not in leaf:
            issues.append(
                _issue(
                    f"l4.{node['id']}", "grid_search requires leaf_config.tuning_grid"
                )
            )
        if search in {"random_search", "bayesian_optimization"}:
            if "tuning_distributions" not in leaf or "tuning_budget" not in leaf:
                issues.append(
                    _issue(
                        f"l4.{node['id']}",
                        f"{search} requires tuning_distributions and tuning_budget",
                    )
                )
        if search == "genetic_algorithm":
            for key in (
                ...
                "genetic_algorithm_generations",
            ):
                if key not in leaf:
                    issues.append(
                        _issue(f"l4.{node['id']}", f"genetic_algorithm requires {key}")
                    )
        if search == "cv_path" and "cv_path_alphas" not in leaf:
            issues.append(
                _issue(
                    f"l4.{node['id']}", "cv_path requires leaf_config.cv_path_alphas"
                )
            )
        if (
            params.get("validation_method") == "kfold"
            and int(leaf.get("n_splits", 5)) > 2
        ):
            issues.append(
                _issue(
                    f"l4.{node['id']}",
                    "kfold validation is rejected for time-series data",
                )
            )
        if (
            params.get("training_start_rule") == "fixed"
            and "fixed_training_end_date" not in leaf
        ):
            issues.append(
                _issue(
                    f"l4.{node['id']}",
                    "fixed training_start_rule requires fixed_training_end_date",
                )
            )
        if (
            params.get("refit_policy") == "every_n_origins"
            and "refit_interval" not in leaf
        ):
            issues.append(
                _issue(f"l4.{node['id']}", "every_n_origins requires refit_interval")
            )
        if params.get("regime_wrapper") and not _has_regime_input(node):
            issues.append(
                _issue(
                    f"l4.{node['id']}",
                    "regime_wrapper requires l1_regime_metadata_v1 input",
                )
            )
    return issues
    ...
    for node in raw.get("nodes", ()):
        ...
        if op == "bivariate_ardl_combination" and len(node.get("inputs", ())) != 2:
            issues.append(
                _issue(
                    f"l4.{node['id']}",
                    "bivariate_ardl_combination requires exactly 2 inputs",
                )
            )
        temporal = params.get("temporal_rule") or params.get(
            "combination_weights_temporal_rule"
        )
...
def _validate_regime(
    raw: dict[str, Any], recipe_context: dict[str, Any] | None
) -> list[Issue]:
    ...
    for node in raw.get("nodes", ()):
        if node.get("op") == "fit" and (node.get("params", {}) or {}).get(
            ...
            return [
                _issue(
                    f"l4.{node['id']}",
                    "regime_wrapper requires L1 regime_definition != none",
                )
            ]
    return []


def _validate_path_average(
    raw: dict[str, Any], recipe_context: dict[str, Any] | None
) -> list[Issue]:
    ...
    for node in raw.get("nodes", ()):
        if node.get("op") == "fit" and (node.get("params", {}) or {}).get(
            ...
            return [
                _issue(
                    f"l4.{node['id']}",
                    "path_average requires L3 target_construction.mode == cumulative_average",
                )
            ]
    return []
...
def _has_regime_input(node: dict[str, Any]) -> bool:
    return any(
        ref == "src_regime" for ref in node.get("inputs", ()) if isinstance(ref, str)
    )


def _ensemble_yaml(
    combine_op: str, combine_params: dict[str, Any], n_inputs: int
) -> str:
    fit_predict = []
    inputs = []
...
_L4_SUBLAYERS = (
    ...
    _CanonicalSubLayerSpec(
        id="L4_B_forecast_strategy", name="Forecast strategy", axes=()
    ),
