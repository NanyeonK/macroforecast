...
class ResolvedExperimentSpec:
    ...
    def validate_compiled_spec(self) -> 'ResolvedExperimentSpec':
        ...
        for key in ['sample_period', 'oos_period']:
...
_MODEL_CLASS_TO_REGISTRY_ID = {
    ...
    'RidgeModel': 'ridge',
    'LassoModel': 'lasso',
    'AdaptiveLassoModel': 'adaptive_lasso',
    'GroupLassoModel': 'group_lasso',
    'ElasticNetModel': 'elastic_net',
    'TVPRidgeModel': 'tvp_ridge',
    'BoogingModel': 'booging',
    'BVARModel': 'bvar',
    'KRRModel': 'krr',
    'SVRRBFModel': 'svr_rbf',
    'SVRLinearModel': 'svr_linear',
    'RFModel': 'rf',
...
def _infer_model_registry_id(exp_cfg: ExperimentConfig) -> str | None:
    if not exp_cfg.model_specs:
        return None
    cls_name = exp_cfg.model_specs[0].model_cls.__name__
    return _MODEL_CLASS_TO_REGISTRY_ID.get(cls_name)
...
def _validate_meta_registries() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    global_defaults = load_global_defaults_registry()
...
def resolve_experiment_spec_from_dict(raw: dict[str, Any], *, preset_id: str | None = None, experiment_overrides: dict[str, Any] | None = None) -> ResolvedExperimentSpec:
    exp_cfg = load_config_from_dict(raw)
    return _build_resolved_spec(exp_cfg, preset_id=preset_id, experiment_overrides=experiment_overrides)
...
def _build_resolved_spec(exp_cfg: ExperimentConfig, *, preset_id: str | None, experiment_overrides: dict[str, Any] | None) -> ResolvedExperimentSpec:
    ...
    target_registry = load_target_registry()
    data_task_registry = load_data_task_registry()
    ...
    validate_preprocessing_registry(preprocessing_registry)
    ...
    config_derived_overrides = {
        'dataset': exp_cfg.data.dataset,
        'target': exp_cfg.data.target,
        'horizon': list(exp_cfg.horizons),
        'outer_window': exp_cfg.window.value,
    }
    ...
    final_experiment_overrides = dict(config_derived_overrides)
    final_experiment_overrides.update(experiment_overrides or {})

    meta_config = resolve_meta_config(
        preset_registry=presets,
        axes_registry=axes,
        preset_id=preset_id,
        global_defaults=global_defaults['global_defaults'],
        dataset_defaults=dataset_defaults,
        target_defaults={**task_defaults, **target_defaults},
        model_defaults=model_defaults,
        experiment_overrides=final_experiment_overrides,
    )
    meta_config.setdefault('benchmark_id', meta_config.get('benchmark_id', 'ar_bic_expanding'))
    ...
    return ResolvedExperimentSpec(
        ...
        data_task_registry=data_task_registry,
        model_registry=model_registry,
        preprocessing_registry=preprocessing_registry,
