...
* ``run`` -- execute a recipe YAML end-to-end (forwards to
  ``macrocast.run``); writes manifest + per-cell artifacts.
* ``replicate`` -- re-run a stored manifest and verify per-cell sink
  hashes match bit-for-bit (forwards to ``macrocast.replicate``).
...
def _cmd_run(args: argparse.Namespace) -> int:
    ...
    print(f"[macrocast run] {recipe_path} -> {output_dir}", file=sys.stderr)
    result = macrocast.run(recipe_path, output_directory=output_dir)
    failed = [c for c in result.cells if c.error is not None]
    print(
        f"[macrocast run] {len(result.cells)} cells "
...
def _cmd_replicate(args: argparse.Namespace) -> int:
    ...
    if not manifest_path.exists():
        ...
        return 2
    print(f"[macrocast replicate] {manifest_path}", file=sys.stderr)
    result = macrocast.replicate(manifest_path)
    summary = {
...
def _cmd_validate(args: argparse.Namespace) -> int:
    ...
    if hard_errors:
        print(f"[macrocast validate] {recipe_path}: {len(hard_errors)} error(s)", file=sys.stderr)
        for err in hard_errors:
            print(f"  ! {err}", file=sys.stderr)
        return 1
    print(f"[macrocast validate] {recipe_path}: OK", file=sys.stderr)
    return 0
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="macrocast",
        description="macrocast CLI -- recipe scaffold wizard, runner, replicator, validator.",
    )
