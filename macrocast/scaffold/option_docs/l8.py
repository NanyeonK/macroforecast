...
_REF_DESIGN_L8 = Reference(
    citation="macrocast design Part 3, L8: 'reproducibility = manifest + provenance + bit-exact replicate.'",
)
...
_PROV_DOCS: dict[str, tuple[str, str, str]] = {
    ...
    "package_version": (
        "macrocast version string.",
        (
            "From ``macrocast.__version__``. Lets ``replicate()`` "
...
register(
    _e("L8_C_provenance", "manifest_format", "json",
       ...
       (
           ...
           "every consumer that uses ``macrocast.replicate``."
       ),
