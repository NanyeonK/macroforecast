...
_REF_DESIGN_L4 = Reference(
    citation="macrocast design Part 2, L4: 'forecasting model is the layer where every authoring iteration ends -- pick family, tune, repeat.'",
)
...
_F_XGB = _f(
    ...
    (
        "Requires ``pip install macrocast[xgboost]``. Histogram-based "
...
_F_LGBM = _f(
    ...
    (
        "Requires ``pip install macrocast[lightgbm]``. Leaf-wise tree "
...
_F_CAT = _f(
    ...
    "Requires ``pip install macrocast[catboost]``. Ordered boosting + native categorical handling.",
...
_F_LSTM = _f(
    ...
    (
        "Requires ``pip install macrocast[deep]``. Sequence-aware RNN "
...
_F_GRU = _f(
    ...
    "Requires ``pip install macrocast[deep]``. Simpler than LSTM (one fewer gate); often comparable on macro panels.",
...
_F_TRANSFORMER = _f(
    ...
    "Requires ``pip install macrocast[deep]``. Self-attention on the lagged feature panel. Single encoder layer; suitable as a non-linear sequence-attention baseline.",
...
_SA_BAYES = _other(
    ...
    "Requires ``pip install macrocast[tuning]`` (optuna). Falls back to ``random_search`` when optuna isn't installed.",
