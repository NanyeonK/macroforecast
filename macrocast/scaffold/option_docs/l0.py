...
_FAILURE_POLICY_FAIL_FAST = OptionDoc(
    ...
    references=(
        Reference(
            citation=(
                "macrocast design Part 1, L0 §A: 'fail_fast vs continue_on_failure "
...
_FAILURE_POLICY_CONTINUE = OptionDoc(
    ...
    references=(
        Reference(
            citation=(
                "macrocast design Part 1, L0 §A: 'continue_on_failure preserves "
...
_REPRODUCIBILITY_SEEDED = OptionDoc(
    ...
    description=(
        ...
        "seed when present.\n\n"
        "This is the only mode under which ``macrocast.replicate(manifest)`` "
    ...
    references=(
        ...
        Reference(
            citation="macrocast PR #215 -- L0 random_seed propagation into L4 random_state.",
        ),
    ),
    when_not_to_use=(
...
_REPRODUCIBILITY_EXPLORATORY = OptionDoc(
    ...
    references=(
        Reference(
            citation=(
                "macrocast design Part 1, L0 §A: 'exploratory mode trades "
...
_COMPUTE_SERIAL = OptionDoc(
    ...
    references=(
        Reference(
            citation=(
                "macrocast design Part 1, L0 §A: 'compute_mode = serial is the "
...
_COMPUTE_PARALLEL = OptionDoc(
    ...
    references=(
        Reference(
            citation="macrocast PR #173 -- cell-level ProcessPoolExecutor.",
        ),
        Reference(
            citation="macrocast PR #204 / #250 -- sub-cell ThreadPoolExecutor.",
        ),
        Reference(
            citation="macrocast PR #279 -- deterministic per-origin seed propagation.",
        ),
    ),
    when_not_to_use=(
