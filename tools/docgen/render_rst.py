...
build time) and as a stand-alone ``python -m macroforecast.scaffold.render_rst``
...
def _format_option(doc: OptionDoc) -> str:
    ...
    if doc.last_reviewed:
        lines.append(f".. note:: Last reviewed {doc.last_reviewed} by {doc.reviewer or 'macroforecast author'}.")
        lines.append("")
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m macroforecast.scaffold.render_rst",
